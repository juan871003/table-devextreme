...
export const additionalColumnPrefix = "additional_column_";
export const numberOfAdditionalColumns = 20;
...
export function generateData(count) {
	...
	for (i = 0; i < count; i++) {
		...
		var item = {
			...
			birthDate: birthDate,
			sampleCurrency: (Math.random() * 100000).toFixed(2),
		};
		for (let j = 0; j < numberOfAdditionalColumns; j++) {
			item[`${additionalColumnPrefix}${j}`] = surnames[random()];
		}
		items.push(item);
	}
	return items;
